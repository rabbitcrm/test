...
console.log('handle hidden columns for DT...' + $('table.bcz-data-table').length);			
  ...
  $("#bcz_search").on('keyup', function () {
	  var bcz_search=$('#bcz_search').val().length;
	  ...
	  if(bcz_search>=3)
	  {
		...
		if (searchString.length <= 0) {
			$('.bcz-search-form .dropdown-menu').html('<li class="padder-l-mini text-center">Specify your query</li>').removeClass('hide');
			return; 
		}
		...
		$.get(appBaseUrl+'search/matchesJson?query='+searchString, function(data, status, jqXHR) {
			...
			if (data.success) {
				...
				for (var i = 0; i < data.results.length; i++) {
					...
					resultsHtml += "<li><a href='"+itemUrl+"'><span class='search-item'>"+data.results[i].name+"</span> <span class='label label-"+labelType+" pull-right'>"+data.results[i].type+"</span></a></li>";
...
console.log(formData);
	...
	if ($('form.bcz-file-upload-form').length) {
		...
		$('form.bcz-file-upload-form input[type=file]').on('change', function() {
	...
	*/
	...
	if($('body.settings').length) {
		...
		$('.users_container1').on('click', 'a.edit_users', function(e) {
			if ($(this).data('bczajax-modal')) {
			  	$.get($(this).data('href'), function(data) {
						$('#edit_user_modal').html(data).modal('show');
			    	$(".select2-option").select2();
		...
		$('.users_container1').on('click', 'a.reset_users', function(e) {
			if ($(this).data('bczajax-modal')) {
			  	$.get($(this).data('href'), function(data) {
						$('#edit_user_modal').html(data).modal('show');
			    	$(".select2-option").select2();
		...
		$('.users_container1').on('click', 'a.delete_users', function(e) {
			if ($(this).data('bczajax-modal')) {
			  	$.get($(this).data('href'), function(data) {
						$('#edit_user_modal').html(data).modal('show');
			    	$(".select2-option").select2();
		...
		$('.users_container1').on('click', 'a.delete_users_modal', function(e) {
			if ($(this).data('bczajax-modal')) {
			  	$.get($(this).data('href'), function(data) {
						$('#edit_user_modal').html(data).modal('show');
			    	$(".select2-option").select2();
...
console.log('select change...');
		...
		$('#collapseTwo').on('change', 'input.bcz-quote-item-field, select.bcz-quote-vat', function() {
			...
			if ($(this).hasClass('bcz-quote-discount')) {
				...
				discPer = (parseFloat(discPer) / price) * 100;
				currItemRow.find('input.bcz-quote-discount1').val(discPer.toFixed(3));
			} else if ($(this).hasClass('bcz-quote-discount1')) {
				var discVal = quotesUtility.toPlainNumber($(this).val());
				discVal = (parseFloat(discVal) * price) / 100;
...
$('body.create-deal .form-horizontal .btn-primary').click(function() {
