...
class BCZ_Controller extends CI_Controller {
	...
	// Datatable columns
	public $leadTableCols = array('lead_name' => 'name', 'company_name' => 'Account', 'owner' => 'owner', 'lead_status' => 'status', 'lead_source' => 'Lead Source', 'lead_create_date' => 'created');
	
		public $CampaignTableCols = array('campaign_name' => 'name', 'type' => 'type', 'owner' => 'owner', 'status' => 'status', 'target_audience' => 'Audience', 'create_date' => 'created');
	
	public $dealTableCols = array('deal_name' => 'name', 'owner' => 'owner','stage' => 'stage', 'deal_amount' => 'amount', 'source' => 'source',  'exp_close' => 'Close Date', 'deal_create_date' => 'created');
	public $taskTableCols = array('task_name' => 'name','owner' => 'user', 'type' => 'type', 'status' => 'status', 'due_date' => 'due Date', 'priority' => 'priority', 'task_create_date' => 'created');
	...
	public $companyTableCols = array('company_name' => 'name', 'owner' => 'owner', 'com_cust_type' => 'type', 'phone' => 'phone', 'website' => 'website', 'company_create_date' => 'created');
	public $productTableCols = array('product_name' => 'name', 'partno' => 'part No', 'category_name' => 'category', 'price' => 'price', 'create_date' => 'created');
	public $caseTableCols = array('case_title' => 'title', 'status' => 'status', 'priority' => 'priority', 'severity' => 'severity', 'company' => 'Account', 'case_create_date' => 'created');
	public $quoteTableCols = array('subject' => 'subject', 'deal_name' => 'Opportunity', 'company_name' => 'Account', 'total' => 'total', 'quote_stage' => 'stage', 'quote_create_date' => 'created');
	public $orderTableCols = array('subject' => 'subject', 'deal_name' => 'Opportunity', 'company_name' => 'Account', 'total' => 'total', 'so_stage' => 'stage', 'estimated_delivery' => 'Delivery');
...
//		session_start();
	...
	public function getAllDealsExport($query,$params) {
		...
		$query="select SQL_CALC_FOUND_ROWS *, if (u.name, u.name, u.name) as owner, c.assign_to as assign_to
						  from deal de 
...
AND uf.org_id != uf.org_id )   where (uf.`org_id`= ? or uf.`org_id`= ? ) AND uf.`campaign_status` != "" ';
	...
	public function getAllTasks() {
		...
		$query="select * from task t ".$query;
	...
	public function getAllCompaniesExport($query,$params) {
		...
		$query="select * from company c ".$query;
	...
	public function getAllContactsExport($query,$params) {
		...
		$query = 'select *, trim(concat(first_name, " ", last_name)) as name from contact co'.$query;
	...
	public function getAllProductsExport($query,$params) {
		...
		$query = 'select * from products pr '.$query;
	...
	public function getAllQuotes() {
		...
		$query = 'select * from quote q '.$query;
	...
	public function getAllOrdersExport($query,$params) {
		...
		$query = 'select SQL_CALC_FOUND_ROWS o.*, de.deal_name, c.company_name, co.first_name, co.last_name, u.user_id, u.name, if (u.name, u.name, u.username) as owner
						  from sales_order o 
						  left join company c on c.company_id = o.company_id 
						  left join contact co on co.contact_id = o.contact_id 
						  left join deal de on de.deal_id = o.deal_id 
						  left join user u on o.so_owner_id = u.user_id '.$query;
	...
	public function getAllCases() {
		...
		$query = 'select * from cases ca '.$query;
	...
	public function getItemQuotes($type, $id) {
		$query = "select SQL_CALC_FOUND_ROWS q.*, de.deal_name, c.company_name, co.first_name, co.last_name, u.user_id, u.name , if (u.name, u.name, u.name) as owner
						  from quote q 
	...
	public function getItemOrders($type, $id) {
		$query = "select SQL_CALC_FOUND_ROWS q.*, de.deal_name, c.company_name, co.first_name, co.last_name, u.user_id, u.name , if (u.name, u.name, u.name) as owner
						  ...
						  left join contact co on co.contact_id = q.contact_id 
						  left join deal de on de.deal_id = q.deal_id 
						  left join user u on q.so_owner_id = u.user_id
						  where q.deal_id = ?";
	...
	public function getContactDetails($id) {
		$resObj = $this->db->query("select *, co.org_id, c.phone as cphone, co.phone from contact co left join company c on co.company_id = c.company_id join user u on co.owner_id = u.user_id where contact_id = ?", array($id));
		return $resObj->row();
	}
			...
			foreach ($rows as $row) {
				...
				foreach ($cols as $colIndex => $col) {
					...
					if ($col == 'actions') {
						...
						{
						$actionsHtml .= '<a href="#delete_user_confirmation_modal" data-toggle="modal" data-action="delete" data-id="'. $row->$indexCol .'"><i class="icon-remove-sign icon-large m-r-small"></i></a>';
						}
	...
	public function constructDTOutput1($rows, $cols, $indexCol = '',$super_admin="", $linkPrefix = '', $linkColIndex = 0) {
		// Arrange data
		$dataRows = array();
		if ($rows[0]) {
			foreach ($rows as $row) {
				...
				foreach ($cols as $colIndex => $col) {
					...
					if ($col == 'actions') {
						...
						$actionsHtml .='<a class="reset_users" href="#reset_user_confirmation_modal" data-toggle="modal" data-action="reset" data-id="'. $row->$indexCol .'"><i class="icon-undo icon-large"></i></a><a href="#deactivate_user_confirmation_modal" data-toggle="modal" data-action="deactivate" data-id="'. $row->$indexCol .'"><i class="icon-lock icon-large"></i></a>';
						if($row->$super_admin!='1')
						{
						$actionsHtml .= '<a class="delete_users" href="#delete_user_confirmation_modal" data-toggle="modal" data-action="delete" data-id="'. $row->$indexCol .'"><i class="icon-remove-sign icon-large m-r-small"></i></a>';
						}
						else
						{
							$actionsHtml .= '<a class="delete_users_modal" href="#delete_modal" data-toggle="modal" data-action="delete"><i class="icon-remove-sign icon-large m-r-small"></i></a>';
						}
	...
	//inactive Users
	public function constructinactiveDTOutput($rows, $cols, $indexCol = '',$super_admin="", $linkPrefix = '', $linkColIndex = 0) {
		// Arrange data
		$dataRows = array();
		if ($rows[0]) {
			foreach ($rows as $row) {
				...
				foreach ($cols as $colIndex => $col) {
					...
					if ($col == 'actions') {
						$actionsHtml = '<a class="edit_users" href="#" data-href="'.base_url().'settings/edituser/'.$row->$indexCol.'" data-bczajax-modal="true" data-action="edit"><i class="icon-edit-sign icon-large m-r-small"></i></a>';
						$actionsHtml .='<a href="#reset_user_confirmation_modal" data-toggle="modal" data-action="reset" data-id="'. $row->$indexCol .'"><i class="icon-undo icon-large"></i></a><a href="#activate_user_confirmation_modal" data-toggle="modal" data-action="deactivate" class="inclass" data-id="'. $row->$indexCol .'"><i class="icon-unlock icon-large"></i></a>';
						if($row->$super_admin!='1')
						{
						$actionsHtml .= '<a href="#delete_user_confirmation_modal" data-toggle="modal" data-action="delete" data-id="'. $row->$indexCol .'"><i class="icon-remove-sign icon-large m-r-small"></i></a>';
						}
						...
						{
							$actionsHtml .= '<a href="#delete_modal" data-toggle="modal" data-action="delete"><i class="icon-remove-sign icon-large m-r-small"></i></a>';
						}
	...
	public function importData($file, $table) {
		$fo = fopen($file, "r"); // Open fiile in read mode

		$rowNo = 0;
		while (($emapData = fgetcsv($fo, "", "\t")) !== FALSE) {

			if ($rowNo) {
				$query = 'insert into ' . $table . ' (' . implode(', ', $tableCols) . ') values (' . $valsStr . ')';
				$res = $this->db->query($query, $emapData);
			} else {
				$tableCols = $emapData;
				$valsStr = '';
...
// echo $entityOrgId . " === " . $this->user->org_id; exit;		
		if ($this->user->org_id != $entityOrgId) show_404();
	}
