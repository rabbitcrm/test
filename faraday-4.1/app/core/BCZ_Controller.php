...
class BCZ_Controller extends CI_Controller {
	...
	// Datatable columns
	public $HomeLeadTableCols = array('lead_name' => 'name', 'owner' => 'owner', 'status' => 'status', 'lead_create_date' => 'created');
	public $HomecompanyTableCols = array('company_name' => 'name', 'owner' => 'owner', 'customer_type' => 'type', 'company_create_date' => 'created');
	public $HomeOppTableCols = array('deal_name' => 'name', 'owner' => 'owner','stage_name' => 'stage','deal_amount' => 'amount');
	public $HomeTopOppCols = array('deal_name' => 'name','deal_amount' => 'amount');
	public $HomeLatestOppCols = array('deal_name' => 'name','deal_amount' => 'amount');
	...
	public $caseTableCols = array('case_title' => 'title','case_no' => 'Ticket No', 'case_status' => 'status', 'priority' => 'priority', 'severity' => 'severity', 'company' => 'Account', 'case_create_date' => 'created');
...
//		session_start();
	...
	public function getAllDealsExport($query,$params) {
						  ...
						  left join user u on (de.deal_owner_id = u.user_id) ".$query;
...
AND uf.org_id != uf.org_id )   where (uf.`org_id`= ? or uf.`org_id`= ? ) AND uf.`campaign_status` != "" ';
	...
	//	echo "SELECT * FROM `user_delete_settings` AS ud INNER JOIN  `user_fields` AS uf  ON ud.user_settings_id != uf.no  WHERE uf.org_id='".$this->user->org_id."' OR  uf.org_id='0' group by uf.no";
		...
		// print_r($fields1);
		 ...
		 if($userwhere=="")
		 {
		  ...
		  $myq="SELECT * FROM `user_fields`  WHERE (org_id='".$this->user->org_id."' OR  org_id='0') AND (".$userwhere.") Order by sort_order ASC";
	...
	// Construct output for datatable
	public function constructDTOutputHome($rows, $cols, $indexCol = '', $linkPrefix = '', $linkColIndex = 0,$type="",$over='') {
		// Arrange data
		$style='';
		if($type=="right")
		{
		...
		if ($rows[0]) {
			foreach ($rows as $row) {
				...
				{
				$name=$row->$col;
						if (strlen($row->$col) > 10) {
							$row->$col = substr($row->$col, 0, 10);
						}
				}
					if($m==1)
					{
						$name=$row->$col;
						if (strlen($row->$col) > 20) {
							
							$row->$col = substr($row->$col, 0, 20);
							$row->$col.="...";
						}
						$colLink = ($colIndex == ($linkColIndex - 1)) ? base_url().$linkPrefix.'/'.$row->$indexCol : '';
						$dataRow[] = ($colLink ? '<a title="'.$name.'" class="text-primary dataover" href="'.$colLink.'">' : '') .$row->$col . (!$colIndex ? '</a>' : '');
					}
					else
					{
						
						$colLink = ($colIndex == ($linkColIndex - 1)) ? base_url().$linkPrefix.'/'.$row->$indexCol : '';
						$dataRow[] = ($colLink ? '<a  class="text-primary dataover" href="'.$colLink.'">' : '') . '<div title="'.$name.'" '.$style.' >'.$row->$col .'</div>'. (!$colIndex ? '</a>' : '');
					}
