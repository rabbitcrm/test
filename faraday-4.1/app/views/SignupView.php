		        ...
		        <div class="col-sm-4 col-sm-offset-4 m-t-large">
		          <section class="panel m-t-large">
		            <header class="panel-heading text-center h4">
		             <a class="" href="http://rabbitcrm.com"><span class=""><img src="<?=base_url().'assets/img/login_logo.jpg';?>" width="" height="40" border="0" title="<?=$this->supportInfo['name']?>" alt="" /></span></a>
		            </header>
		            <div class="panel-body">

	            	  <? $this->load->view('MessagesView', array(messages => $messages)) ?>

		            	<form action="<?=base_url()?>createuser" class="form-horizontal" method="post" data-validate="parsley">
		              <div class="block">
		                <label class="control-label">Name<?=$this->mandatoryFieldIndicator?></label>
		                <input type="text" name="name" data-required="true" placeholder="Your name" class="form-control" value="<?=$name?>">
		              </div>
		              <div class="block">
		                <label class="control-label">Email<?=$this->mandatoryFieldIndicator?></label>
		                <input type="email" name="user_email" data-required="true" placeholder="test@example.com" class="form-control" value="<?=$user_email?>">
		              </div>
		              <div class="block">
		                <label class="control-label">Password<?=$this->mandatoryFieldIndicator?></label>
		                <input type="password" name="password" data-required="true" id="inputPassword" class="form-control">
                        <input type="hidden" name="super_admin" value="1">
		              </div>
<!--
		              <div class="checkbox">
		                <label>
		                  <input type="checkbox" name="terms" data-required="true" <?php //if ($terms) { ?>checked='checked'<?php //} ?>> Agree the terms and policy
		                </label>
		              </div>
-->
		              <button type="submit" class="btn btn-info m-t">Sign up</button>
<?php /*?>		              <div class="line line-dashed"></div>
		              <p class="text-muted text-center"><small>Already have an account?</small></p>
		              <a href="<?=base_url()?>login" class="btn btn-white btn-block">Sign in</a><?php */?>
		            	</form>
		            </div>
		          </section>
		        </div>
