...
<html lang="en" class="no-js">
	...
	<body class="bclouds <?=$this->bodyClass;?>">
	  	<!-- header -->
	  	<?php /*?><header id="header" class="navbar bg bg-black"><?php */?>
		        ...
		        <div class="col-sm-4 col-sm-offset-4 m-t-large">
		          <section class="panel">
		            ...
		            <div class="panel-body">
		              ...
		              <form action="<?=base_url()?>login/submit" class="form-horizontal" method="post" data-validate="parsley">
		                <div class="block">
		                  	<label class="control-label">Email<?=$this->mandatoryFieldIndicator?></label>
	                    	<input type="email" name="email" data-required="true" data-rule-email="true" class="form-control <?=$submit ? (($messages['error']['username'] || $loginFailed) ? 'parsley-error' : '') : ''?>" value="<?=$email?>">
		                </div>
		                <div class="block">
		                  	<label class="control-label">Password<?=$this->mandatoryFieldIndicator?></label>
	                    	<input type="password" name="password" data-required="true" class="form-control <?=$submit ? (($messages['error']['password'] || $loginFailed) ? 'parsley-error' : '') : ''?>">
		                </div>

		              	<a href="<?=base_url()?>reset" class="pull-right m-t-mini"><small>Forgot password?</small></a>
		              	<button type="submit" class="btn btn-info">Sign in</button>
		              <?php /*?>	<div class="line line-dashed"></div>
		              	<p class="text-muted text-center"><small>Do not have an account?</small></p>
		              	<a href="<?=base_url()?>signup" class="btn btn-white btn-block">Create an account</a><?php */?>
		              </form>
		            </div>
		          </section>
		        </div>
