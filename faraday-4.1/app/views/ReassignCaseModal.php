<!-- .modal -->
<div id="reassign_case_modal" class="modal fade">
  <form class="form-horizontal" method="post" data-validate="parsley" action="<?=base_url()?>cases/reassign">
    <div class="modal-dialog">
      <div class="modal-content"><!-- .modal-content -->
            ...
            <div class="panel-body text-center">
              ...
              <div class="btn-group col-sm-4">
                <button data-toggle="dropdown" class="btn btn-sm btn-white dropdown-toggle col-sm-12">
                  <span class="dropdown-label col-sm-11 text-left no-padder"><?=$case->assignee?$case->assignee:($this->user->name?$this->user->name:$this->user->username)?></span>
                  <span class="caret"></span>
                </button> 
                <ul class="dropdown-menu dropdown-select col-sm-12 text-left">
                  <?php foreach($users as $user) { if ($case->assign_to != $user->user_id) { ?>
        ...
        <div class="modal-footer">
          ...
          <button type="button" id="reassign_case" class="btn btn-sm btn-primary bcz-submit-btn" data-loading-text="Reassigning...">Reassign</button>
        </div>
      </div><!-- /.modal-content -->
    </div>
  </form>
