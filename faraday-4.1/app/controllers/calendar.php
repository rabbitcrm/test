...
class calendar extends BCZ_Controller {
	...
	public function index()
	{
		...
		$this->bodyClass = $this->pageTitle = $this->pageDesc = 'Calendar';
		...
		if (isset($_REQUEST['filters'])) {
			$data['sourcePath'] = 'tasks/getTasksjson1';
			$this->load->view('DataTableView', $data);
		} else {
			$data['filters'] = array(
			'User' => array('col' => 'task_owner_id', 'alias' => 'name'),
			...
			$data['getCurrTime']=$this->convertDateTime($this->getCurrTime(),'Y/m/d');
			
			$data['content'] = 'CalendarView';
	...
	public function getalltasksjson() {
		...
		$tasks = $this->getTasks1();
		$output = $this->constructDTOutput($tasks, array_keys($this->taskTableCols), '', '');
...
$task->statusList =$statusList;
 ...
 $data['type']= $this->getStatusName($task->type,'task_type');
	...
	public function getTasks($filters = '') {
		...
		$query = "select SQL_CALC_FOUND_ROWS * , t.description as task_name, t.task_name as description,  if (u.name, u.name, u.name) as owner
						  from task t 
		...
		{
		if ($this->isManager) {
			$whereCond .= ' and (t.org_id = ? or t.org_id = ? ) and (t.task_report_to_id = ? or t.task_owner_id = ?) AND uf.task_status!=? ';
			...
			$whereCond .= ' and (t.org_id = ? or t.org_id = ?) and t.task_owner_id = ? AND uf.task_status!=? ';
			$params[] = $this->user->org_id;
			$params[] = "0";
			$params[] = $this->user->user_id;
			$params[] = "Completed";
		} elseif ($this->isAdmin && $this->user->org_id) {
			$whereCond .= ' and (t.org_id = ? or t.org_id = ?) AND uf.task_status!=? ';
			$params[] = $this->user->org_id;
			$params[] = "0";
			$params[] = "Completed";
		}
		...
		{
			if ($this->isManager) {
			$whereCond .= ' and t.org_id = ? and (t.task_report_to_id = ? or t.task_owner_id = ?)  AND uf.task_status!=? ';
			$params[] = $this->user->org_id;
			$params[] = $this->user->user_id;
			$params[] = $this->user->user_id;	
			$params[] = "Completed";	
		} elseif ($this->isExecutive) {
			$whereCond .= ' and t.org_id = ? and t.task_owner_id = ? and t.task_owner_id = ?  AND uf.task_status!=?';
			$params[] = $this->user->org_id;
			$params[] = $this->user->user_id;
			$params[] = "Completed";
		} elseif ($this->isAdmin && $this->user->org_id) {
			$whereCond .= ' and t.org_id = ?  AND uf.task_status!=?';
			$params[] = $this->user->org_id;
			$params[] = "Completed";
		}
		}
		//$whereCond.=" AND t.due_date >= '".date('Y-m-d')."' ";
		...
		if ($resObj->num_rows()) {
			...
			foreach ($resObj->result() as $row) {
				...
				$row->due_time=date("H:i:s", strtotime($row->due_time));
				$row->Start = $row->due_date."T".$row->due_time;
				
				if($row->associate_to=="deal")
				{
					$row->url=base_url()."deal/".$row->task_id;
					
					$row->associate_to="Opportunity";
				}
				else if($row->associate_to=="company")
				{
					$row->url=base_url()."company/".$row->task_id;
					
					$row->associate_to="Accounts";
				}
				else
				{
					$row->url= base_url().$row->associate_to."/".$row->task_id;
				}
				
				$row->url=base_url()."tasks/details/".$row->task_id;
				$row->associate_to = $row->associate_to;
				$row->due_date = $this->convertDateTime($row->due_date,"Y-m-d");
				...
				$type		=	$row->associate_to ;
				if($type=='Opportunity')
				{
				 $associate_id	=	$row->associate_id ;
				
				$query1 =	"select * from deal i 
							inner join company c on c.company_id=i.deal_company_id
							WHERE deal_id=?";
							
				$params1 = array();
				$params1[]=$associate_id;
				$resObj1 = $this->db->query($query1, $params1);
				$resObj1->num_rows();
				if ($resObj1->num_rows()) {
				foreach ($resObj1->result() as $row1) {
				 $row->associate_to.="-".$row1->company_name;
				 $row->associate_to= str_replace("'","",$row->associate_to);
	...
	public function getTasks1($filters = '') {
		if (!$filters) $filters = $this->filters;

		$query = "select if (u.name, u.name, u.name) as owner , t.task_id as TaskID ,u.user_id as OwnerID , t.task_name as Title , t.description as Description, t.task_create_date as StartTimezone, t.task_create_date as Start, t.task_create_date as End,u.user_id as RecurrenceRule
						  from task t 
						  left join user u on t.task_owner_id = u.user_id";
		...
		$sOrder = "";
			...
			$sOrder = "ORDER BY  ORDER BY STR_TO_DATE(due_time,'%h.%i%p');";
		...
		if ($resObj->num_rows()) {
			...
			foreach ($resObj->result() as $row) {
/*				
				$row->StartTimezone = strtotime($this->convertDateTime('9:00'));
				$row->Start = "/Date(".strtotime($this->convertDateTime($row->Start)).")/";
				$row->End = "/Date(".strtotime($this->convertDateTime($row->End)).")/";
				$row->task_create_date = $this->convertDateTime($row->task_create_date);*/
				$tasks[] = $row;
