...
// echo "<pre>"; print_r($this->user); exit;		
	...
	public function index()
	{
		...
		if (isset($_REQUEST['filters'])) {
			...
			$data['filters'] = array('owner' => array('col' => 'user_id', 'alias' => 'owner', 'user_status' => 'user_status'), 
															 'status' => array('col' => 'lead_status', 'alias' => 'lead_status_name'),													 														 'created' => array('col' => 'lead_create_date', 'alias' => 'lead_create_date', 'type' => 'date'),															 'Modified' => array('col' => 'lead_modify_date', 'alias' => 'lead_modify_date', 'type' => 'date'));
			...
			$data['leads_filters'] = $this->getLeadsfilters();
	...
	public function import() {
		...
		$query="DELETE FROM `lead_tmp1` WHERE org_id = ?";
	...
	public function importcsv() {
		...
		$lead_tmp_TableCols="`org_id` ,`title` ,`first_name` ,`last_name` ,`designation` ,`phone` ,`email`,`company_name` ,`bill_addr1` ,`bill_city` ,`bill_state` ,`bill_postal_code` ,`bill_country`,`lead_create_date`,`lead_modify_date`";
	...
	public function mapping() {
		...
		if ($resObj->num_rows()) {
			foreach ($resObj->result() as $row) {
				$data['lead_table_data']=array (array ('name' => 'title','col' => 'title','value'=>$row->title),array ('name' => 'First Name','col' => 'first_name','value'=>$row->first_name),array ('name' => 'Last Name','col' => 'last_name','value'=>$row->last_name),array ('name' => 'Designation','col' => 'designation','value'=>$row->designation),array ('name' => 'Phone','col' => 'phone','value'=>$row->phone),array ('name' => 'Email','col' => 'email','value'=>$row->email),array ('name' => 'Company Name','col' => 'company_name','value'=>$row->company_name),array ('name' => 'Address','col' => 'bill_addr1','value'=>$row->title),array ('name' => 'City','col' => 'bill_city','value'=>$row->bill_city),array ('name' => 'State','col' => 'bill_state','value'=>$row->bill_state),array ('name' => 'Postal Code','col' => 'bill_postal_code','value'=>$row->bill_postal_code),array ('name' => 'Country','col' => 'bill_country','value'=>$row->bill_country));
			}
		}
		$this->load->view('FirstLayoutView', $data);
	}
	...
	public function importmapping() {
		...
		$report_to_id = $this->getUserReporter($_POST['lead_owner_id']);	// Get reporter id
		$lead_owner_id = $_POST['lead_owner_id'];
		$lead_status = $_POST['lead_status'];
		$lead_source = $_POST['lead_source'];
		
		unset($_POST['lead_owner_id']);
		unset($_POST['lead_status']);
		unset($_POST['lead_source']);
		
			$query = 'update lead_tmp1 set lead_owner_id= ? , report_to_id= ? , lead_status= ? ,lead_source= ? where org_id = ? ' ;
		...
		$res = $this->db->query($query, array($lead_owner_id,$report_to_id,$lead_status,$lead_source,$this->user->org_id));
...
$params = array();
		...
		$query="INSERT INTO lead (lead_owner_id,report_to_id,org_id,lead_create_date,lead_modify_date,lead_status,lead_source".$valsStr.") (SELECT lead_owner_id,report_to_id,org_id,lead_create_date,lead_modify_date,lead_status,lead_source ".$table_data." from lead_tmp1 WHERE org_id = ?) ";
		...
		redirect(base_url() . 'leads/' , 'location', 301);
	}
...
// Get task status
		...
		foreach ($data['fields'] as $field) { 
			...
			if ($field->lead_status) {
				 $statusList[$field->no] = $field->lead_status;
		...
		if($this->user->demo==0)
		{
			...
			$data['NextId']=$this->getNextIdAndPreviousId("select le.*, u.name from lead le left join user u on le.lead_owner_id = u.user_id where (le.lead_id > ?) AND (le.org_id = ? OR  le.org_id = '0' )  ORDER BY le.lead_id ASC LIMIT 1",$id,"lead_id",$org_id);

			$data['PreviousId']=$this->getNextIdAndPreviousId("select le.*, u.name from lead le left join user u on le.lead_owner_id = u.user_id where (le.lead_id < ? ) AND (le.org_id = ? OR  le.org_id = '0' )  ORDER BY le.lead_id desc LIMIT 1",$id,"lead_id",$org_id);
		}
		else
		{
			
				$data['NextId']=$this->getNextIdAndPreviousId("select le.*, u.name from lead le left join user u on le.lead_owner_id = u.user_id where le.lead_id > ? AND le.org_id = ?  ORDER BY le.lead_id ASC LIMIT 1",$id,"lead_id",$org_id);
		
	$data['PreviousId']=$this->getNextIdAndPreviousId("select le.*, u.name from lead le left join user u on le.lead_owner_id = u.user_id where le.lead_id < ? AND le.org_id = ?  ORDER BY le.lead_id desc LIMIT 1",$id,"lead_id",$org_id);
		}
		...
		$data['status']= $this->getStatusName($lead->lead_status,'lead_status');
		$data['customer_type']= $this->getStatusName($lead->customer_type,'customer_type');
		// Lead notes
...
// echo "<pre>"; print_r($data['history']); exit;		
	...
	public function edit($id, $data = array()) {
		...
		$data['lead_status']= $this->getStatusName($data['lead']->lead_status,'lead_status');
		
		$data['customer_type']= $this->getStatusName($data['lead']->customer_type,'customer_type');
	...
	public function getLeads($filters = '') {
		...
		$query = "select SQL_CALC_FOUND_ROWS *,u.user_status as user_status, trim(concat(le.first_name, ' ', le.last_name)) as lead_name, if (u.name, u.name, u.name) as owner ,uf1.source as lead_source ,if(uf.lead_status,uf.lead_status,uf.lead_status) as lead_status_name,if(le.lead_status,le.lead_status,le.lead_status) as lead_status
						  from lead le
		...
		{
		if ($this->isManager) {
			$whereCond .= ' where (le.org_id = ? and le.report_to_id = ? or le.lead_owner_id = ?) or (le.org_id = ? and le.report_to_id = ? or le.lead_owner_id = ?)';
			$params[] = $this->user->org_id;
			...
			$whereCond .= ' where (le.org_id = ? and le.lead_owner_id = ? ) or (le.org_id = ? and le.lead_owner_id = ? )';
			$params[] = $this->user->org_id;
			$params[] = $this->user->user_id;
			$params[] = "0";
			$params[] = $this->user->user_id;
			...
			$whereCond .= ' where (le.org_id = ? or le.org_id = ? )  ';
			$params[] = $this->user->org_id;
			$params[] = "0";
		...
		$archived='AND uf.lead_status!="Archived"';
		 ...
		 */
$whereCond=$whereCond.$archived;

$countWhereCond=$countWhereCond.$archived;
		...
		$countQuery = 'SELECT COUNT(`lead_id`) as totalRows FROM lead le left join user_fields uf on le.lead_status =uf.no ' . $countWhereCond;
	...
	public function getLeadsfilters() {
	
		if (!$filters) $filters = $this->filters;

		$query = "select SQL_CALC_FOUND_ROWS *,u.user_status as user_status, trim(concat(le.first_name, ' ', le.last_name)) as lead_name, if (u.name, u.name, u.name) as owner ,uf.source as source ,le.lead_source as lead_source ,if(uf.lead_status,uf.lead_status,uf.lead_status) as lead_status_name,if(le.lead_status,le.lead_status,le.lead_status) as lead_status
						  from lead le
						  left join user_fields uf on le.lead_status =uf.no 
						  left join user_fields uf1 on le.lead_source =uf1.no 
						  left join user u on le.lead_owner_id = u.user_id  ";
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( $sOrder == "ORDER BY" )
			{
				$sOrder = "ORDER BY uf.sort_order ASC";
			}
		}
		$sOrder = "ORDER BY uf.sort_order ASC";
...
$countWhereCond=$countWhereCond.$archived;
	...
	public function getLeadDetails($id) {
		$resObj = $this->db->query("select le.*, u.name ,uf1.source as lead_source from lead le left join user u on le.lead_owner_id = u.user_id  left join user_fields uf1 on le.lead_source =uf1.no  where le.lead_id = ?", array($id));
		return $resObj->row();
	}
