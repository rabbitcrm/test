...
// echo "<pre>"; print_r($this->user); exit;		
	...
	public function details($id) {
		...
		foreach ($data['fields'] as $field) { 
			...
			if ($field->campaign_status) {
				 $statusList[$field->no] = $field->campaign_status;
				if ($campaign->statuss == $field->campaign_status) $campaignStatusIndex = $count;
				$count++;
				
			}
		}
		
		 $campaign->stages =$statusList;
	 
		 $data['status']= $this->getStatusName($campaign->status,'campaign_status');
	...
	public function getCampaign($filters = '') {
		if (!$filters) $filters = $this->filters;
						  	$query = "select SQL_CALC_FOUND_ROWS *, if (u.name, u.name, u.name) as owner ,u.user_status as user_status , uf.campaign_type  as campaign_type_name ,ca.campaign_type as  campaign_type , uf1.campaign_status as campaignstatus,ca.campaign_status as campaign_status
						  from campaign ca 
						  left join user u on ca.owner_id = u.user_id 
						  left join user_fields uf on ca.campaign_type = uf.no
						  left join user_fields uf1 on ca.campaign_status = uf1.no";
	...
	public function getCampaignDetails($id) {
		$resObj = $this->db->query("select ca.*, u.name, uf.campaign_type  as campaign_type ,uf1.campaign_status as campaign_status,ca.campaign_status as status from campaign ca left join user u on ca.owner_id = u.user_id left join user_fields uf on ca.campaign_type = uf.no left join user_fields uf1 on ca.campaign_status = uf1.no where ca.campaign_id = ?", array($id));
	...
	public function changestatus() {
		// Update task
		$updateQuery = 'update campaign set campaign_status = ? where campaign_id = ?';
