...
if($_SESSION['filters']!="contacts")
		...
		if (isset($_REQUEST['filters'])) {
			...
			$data['filters'] = array('owner' => array('col' => 'assign_to', 'alias' => 'owner', 'user_status' => 'user_status'), 'created' => array('col' => 'contact_create_date', 'alias' => 'contact_create_date', 'type' => 'date'),															 'Modified' => array('col' => 'contact_modify_date', 'alias' => 'contact_modify_date', 'type' => 'date'));
...
$params = array();
	...
	public function submit() {
		...
		$valsStr .= ($valsStr ? ', ' : '') . '?';
		$formFields['assign_to'] = $this->user->user_id;
		$valsStr .= ($valsStr ? ', ' : '') . '?';
	...
	public function modalsubmit() {
		...
		$names=$title.$first_name." ".$last_name;
		...
		if ($res) { # && $comRes) {
			...
			$this->output
	    		...
	    		->set_output(json_encode(array('contactId' => $contactId, 'names' => $names)));
	...
	public function getContacts($filters = '',$filters_search = '') {
		...
		$query = "select SQL_CALC_FOUND_ROWS *,u.user_status as user_status, c.phone as cphone, co.phone, trim(concat(co.first_name, ' ', co.last_name)) as contact_name, u.name as owner, co.assign_to as assign_to
						  from contact co 
						  left join company c on co.company_id = c.company_id 
						  left join user u on co.assign_to = u.user_id";
	...
	public function getsearchContacts($filters) {
		...
		$query = "select SQL_CALC_FOUND_ROWS *, c.phone as cphone, co.phone, trim(concat(co.first_name, ' ', co.last_name)) as contact_name, if (u.name, u.name, u.name) as owner, co.owner_id as owner_id  ,u.user_status as user_status
						  from contact co 
						  left join company c on co.company_id = c.company_id 
